...
data_root = Path('/Users/antonfirc/Documents/Skola/PHD/Publikace/2022/ESORICS-Nine_simple_tricks/IDSD/dataset/JSUT')
rec_path = data_root.joinpath('basic5000').joinpath('wav')
...
np.save(f'processed_data/JSUT_mel.npy', mels_np, allow_pickle=True, fix_imports=True)

stfts_np = np.array(stfts)
np.save(f'processed_data/JSUT_stft.npy', stfts_np, allow_pickle=True, fix_imports=True)

cqts_np = np.array(cqts)
np.save(f'processed_data/JSUT_cqt.npy', cqts_np, allow_pickle=True, fix_imports=True)

vqts_np = np.array(vqts)
np.save(f'processed_data/JSUT_vqt.npy', vqts_np, allow_pickle=True, fix_imports=True)

iirts_np = np.array(iirts)
np.save(f'processed_data/JSUT_iirt.npy', iirts_np, allow_pickle=True, fix_imports=True)

mfccs_np = np.array(mfccs)
np.save(f'processed_data/JSUT_mfcc.npy', mfccs_np, allow_pickle=True, fix_imports=True)

chromas_np = np.array(chromas)
np.save(f'processed_data/JSUT_chroma.npy', chromas_np, allow_pickle=True, fix_imports=True)
