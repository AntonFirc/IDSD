...
for feature in features:
    score_save_path = data_root.joinpath('cross-dataset').joinpath('F2S-train')

    gen_score_f_name = score_save_path.joinpath(f'cross-dataset-eval-{feature}-eval-genuine.txt')
    ...
    df_score_f_name = score_save_path.joinpath(f'cross-dataset-eval-{feature}-eval-spoof.txt')
    ...
    eer_save_path = score_save_path.joinpath(feature)
    ...
    eerinf_cmd = [
        ...
        '-e',
        f'Cross-dataset validation - {feature}',
        '-sp',
        str(eer_save_path)
    ]
