...
dataset = 'JSUT'
dir_type = 'training'
data_root = Path(f'../JSUT/{dir_type}')#/Users/antonfirc/Documents/Skola/PHD/Tools/Dataset/FoR/for-2seconds/{dir_type}')
rec_folders = ['real', 'fake']
processed_data = 'processed_data_JSUT'
mels, stfts, cqts, vqts, iirts, mfccs, chromas = [], [], [], [], [], [], []

try:
    opts, args = getopt.getopt(sys.argv[1:], "d:")
except getopt.GetoptError:
    print('python3 data_proprocessing_for.py -d <subdir>')
    sys.exit(2)
for opt, arg in opts:
    if opt == '-d':
        dir_type = arg
        data_root = Path(f'../JSUT/{dir_type}')
...
np.save(f'{processed_data}/{dataset}_{dir_type}_mel.npy', mels_np, allow_pickle=True, fix_imports=True)

stfts_np = np.array(stfts)
np.save(f'{processed_data}/{dataset}_{dir_type}_stft.npy', stfts_np, allow_pickle=True, fix_imports=True)

cqts_np = np.array(cqts)
np.save(f'{processed_data}/{dataset}_{dir_type}_cqt.npy', cqts_np, allow_pickle=True, fix_imports=True)

vqts_np = np.array(vqts)
np.save(f'{processed_data}/{dataset}_{dir_type}_vqt.npy', vqts_np, allow_pickle=True, fix_imports=True)

iirts_np = np.array(iirts)
np.save(f'{processed_data}/{dataset}_{dir_type}_iirt.npy', iirts_np, allow_pickle=True, fix_imports=True)

mfccs_np = np.array(mfccs)
np.save(f'{processed_data}/{dataset}_{dir_type}_mfcc.npy', mfccs_np, allow_pickle=True, fix_imports=True)

chromas_np = np.array(chromas)
np.save(f'{processed_data}/{dataset}_{dir_type}_chroma.npy', chromas_np, allow_pickle=True, fix_imports=True)
